...
# Date:		03-06-2015					
# Version       1.6		
# - Change email subject : ESXi host monitoring
# - Remove powerstate change function
# - Combine username/password and host IP to 1 file : IP,username,password(encrypted)		
###########################################################################################
...
Function TestPort-VM
{
    ...
    If ($Protocol -eq 'TCP')
    {
        ...
        Catch
        {
            ...
            echo "Cannot connect to $Names - $VMs port $Port" | Out-File .\report.txt -Append 
