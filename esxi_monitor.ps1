...
#   "your ESXi host password" | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString
#   and copy the output code to password column in information.csv
...
Function CheckRDP_QA
{
    Param($Host_IP)
    #Get-VM | Select Name, PowerState, @{N="ip";E={@($_.guest.IPAddress[0])}}, @{N="toolstatus";E={@($_.ExtensionData.Guest.ToolsVersionStatus)}},  @{N="toolversion";E={@($_.ExtensionData.config.tools.toolsVersion)}} | Export-Csv vm_ip.csv -NoTypeInformation
    $vm_on = Get-VM | Where-Object {$_.PowerState -eq "PoweredOn"}
If (Test-Path .\QA_VMs_list.csv)
{
            Import-Csv QA_VMs_list.csv |  
            foreach {  
            $VMs = $_.ip
            $Names = $_.Name
            $port = $_.port
            $terminal = $_.terminal
                ...
                If (($port) -and ($vm_on.Name -contains $Names))
                {
                    #Test RDP for each VM
                    TestPort-VM -IPAddress $terminal -Port $port -Protocol TCP 
                }
                Else
                {
                    Write-Host "$Names does not have a specific port to remote or $Names is not powered on" 
                }
       
    } #end for each VMs loop  

} #End if exist QA_VMs_list.csv

Else 
{
Write-Host "Cannot find QA_VMs_list.csv, please check again!!!"
}

} #End function CheckRDP_QA
...
#########################
        ...
        Else    
        {
            ...
            #Check RDP to each powered on VM
                ...
                { 
                    "PreQA" {CheckRDP $Host_IP} 
                    "QA" {CheckRDP_QA $Host_IP} 
                    #"DEV" {Currently we don't check DEV ESXi host} 
                    default {"The environment could not be determined."}
                }   
            }  
        }
...
###########################
    ...
    { 
        Send-MailMessage -to $recipients -Cc $cc_recipients -from "Monitor Script <monitorscript@aswigsolutions.com>" -SmtpServer $smtp_server -Subject "[ALERT] [$environment] ESXi host monitoring" -priority High -Credential $mycreds -BodyAsHtml "$content"
    }
    Else 
    {
